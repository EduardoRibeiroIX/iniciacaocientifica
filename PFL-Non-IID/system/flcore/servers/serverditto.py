...
class Ditto(Server):
    ...
    def treinamento(self, i, clientes):
        ...
        self.ids.extend(self.receive_models())
    ...
    def train(self):
        ...
        for i in range(self.global_rounds+1):
            if i == 0:
                ...
                df_cluster_clientes = self.cluster_MiniBatchKMeans(df_clientes, k)
                ...
                df_clientes = self.csv_clients(self.users)
                df = self.cluster_MiniBatchKMeans(df_clientes, k)
                self.users = []
                self.treinamento(i, self.selected_clients)
                df = self.updated_data(df, clientes_cluster[0], self.users)
                df_cluster_clientes = self.cluster_MiniBatchKMeans(df, k)
